...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWndEx)
	...
	ON_COMMAND(ID_PROCESS_FILES, &CMainFrame::OnProcessFiles)
	ON_WM_SETTINGCHANGE()
END_MESSAGE_MAP()
...
void CMainFrame::OnProcessFiles()
{
	...
	CSynonymDictionaryAppDoc* pDoc = (CSynonymDictionaryAppDoc*)GetActiveDocument();
	if (pDoc)
	{
		pDoc->ProcessFiles("input.txt", "synonyms.txt", "output.txt");
		AfxMessageBox(L"Обработка завершена");
	}
	else
	{
		AfxMessageBox(L"Документ не открыт");
	}
}
